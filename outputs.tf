...
output "storage_accounts_queue_properties" {
  ...
  value       = { for k, v in azurerm_storage_account.storage_accounts : k => one(v.queue_properties) if v.queue_properties != null && length(v.queue_properties) > 0 }
}
...
output "storage_accounts_static_website" {
  ...
  value       = { for k, v in azurerm_storage_account.storage_accounts : k => one(v.static_website) if v.static_website != null && length(v.static_website) > 0 }
}
